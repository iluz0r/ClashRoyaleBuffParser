...
public class BuffParser {
	...
	private void parseRrsLong(StringBuilder res) {
		res.append("RRSLONG:");
		res.append("\n");
		try {
			RrsLong rrsLong = readRrsLong();
			res.append("high value: " + rrsLong.hi);
			res.append("\n");
			res.append("low value: " + rrsLong.lo);
		} catch (Exception ignored) {
			ignored.printStackTrace();
		}
		res.append("\n\n");
	...
	public SLong readLong() {
		byte[] l = new byte[mByteBuffer.capacity()];
		mByteBuffer.get(l, 0, mByteBuffer.capacity());
	...
	public RrsLong readRrsLong() {
		int start = 0;
		int count = 0;
		long hivalue = 0;
		do {
			byte[] hiBytes = Arrays.copyOfRange(mPayload, start++, ++count);
			ByteBuffer p = ByteBuffer.wrap(hiBytes);
			SCUtils.RrsInt32 hi = SCUtils.readRrsInt32(p);
			hivalue = hi.value;
		} while (!(hivalue > 0));

		byte[] loBytes = Arrays.copyOfRange(mPayload, count, mPayload.length);
		ByteBuffer p = ByteBuffer.wrap(loBytes);
		SCUtils.RrsInt32 lo = SCUtils.readRrsInt32(p);
		long lovalue = lo.value;

		return new RrsLong(hivalue, lovalue);
	}
	...
	public static class RrsLong {
		public long hi;
		public long lo;

		public RrsLong(long hi, long lo) {
			this.hi = hi;
			this.lo = lo;
		}
	}
